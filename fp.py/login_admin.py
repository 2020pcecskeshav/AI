...
def submit():
    ...
    if len(op) > 0:
        ...
        if userid == 'admin' and password == p:
            ...
            mb.showinfo('sucess','login successfully')
            
            
            if op=='register':
                from tinkter.simpledialog import askstring
                name=askstring('Name ','for whom you want')
                import register_face as register_face
                rf.register(name)
            elif op=='clear':
                import clear_data
                print('clear data')
